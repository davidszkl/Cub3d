...
/*   Updated: 2021/12/07 12:58:32 by mlefevre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	t_raytrace_return	r;
	const t_vec2		p = rotate(dir, PI / 2);
	t_vec2				dir;
	int					x;
	...
	while (++x < SIZE_X)
	{
		dir.x = main->dir.x / 2. + p.x * (x - SIZE_X / 2.) / (float)SIZE_X;
		dir.y = main->dir.y / 2. + p.y * (x - SIZE_X / 2.) / (float)SIZE_X;
		dir = unit(dir);
		r = raytrace(main->player_pos, dir, main->map, main->map_dim);
		draw_wall(main, x, r);
	}
